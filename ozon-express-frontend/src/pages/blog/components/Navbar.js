...
import SearchBlogCardSkeleton from './SearchBlogCardSkeleton'
...
function Navbar({ HandleInputChange, searchBoxOpen, isLoading, searchResult }) {
  ...
  const Links = [
    { title: "Home", url: '/' },
    { title: "OZON Market", url: '/market', style: 'font-bold' },
  ]
  ...
  return (
    <nav className={`h-fit lg:h-20 fixed top-0 left-0 w-full z-50 transition-all duration-300 ${fix && 'bg-white dark:bg-black'}`}>
      <div className='container relative mx-auto px-5 md:px-10 xl:px-20 py-3 lg:flex lg:items-center justify-between transition-all duration-500 ease-in'>
        ...
        <div className={`relative flex flex-col items-center`}>
          <div className="hidden lg:flex items-center space-x-2 px-4 py-2 bg-ozon-gray text-sm text-gray-700 w-[450px] h-9 rounded-full dark:bg-ozon-dark-gray dark:text-white">
            <span><BsSearch /></span>
            <input type="text" className='bg-transparent w-full focus:outline-none' placeholder='Rechercher des blogs ...' onChange={HandleInputChange} />
          </div>
          <SearchBox
            searchBoxOpen={searchBoxOpen}
            isLoading={isLoading}
            searchResult={searchResult}
            skeleton={<SearchBlogCardSkeleton />}
            cards={searchResult?.map((item, index) => (
              <SearchBlogCard blog={item} key={index} />
            ))}
          />
        </div>
