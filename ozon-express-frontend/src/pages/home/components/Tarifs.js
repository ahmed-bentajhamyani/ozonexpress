...
function Tarifs({ setTarifsLoaded, isLoaded }) {
  ...
  useEffect(() => {
    if (tarifs[0] && agencesDep[0] && agencesArr[0]) setTarifsLoaded(true);
  }, [agencesDep, agencesArr]);
  ...
  useEffect(() => {
    if (values.agenceDepId && values.agenceArrId) {
      const tarif = tarifs.filter(function (tarif) {
        return tarif.agenceDepId.toString() === values.agenceDepId && tarif.agenceArrId.toString() === values.agenceArrId;
      });
                  ...
                  <p className="mb-0"><span className='font-medium text-white'>Sélectionnez la ville de départ :</span><br />
                    <SelectTag
                      ...
                      tag={{
                        ...
                        optionStyle: 'block w-full text-sm text-black border rounded py-2 px-4 mb-3 focus:outline-none focus:border-ozon-red dark:focus:border-ozon-yellow',
